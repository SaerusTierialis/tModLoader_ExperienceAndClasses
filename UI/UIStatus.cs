        ...
        public void Update() {
            ...
            if (status_to_draw.Count > 0) {
                if (Shortcuts.Now.CompareTo(time_next_update) > 0) {
                    ...
                    time_next_update = Shortcuts.Now.AddSeconds(SECONDS_BETWEEN_UPDATE);
                }
            }
            ...
            if (needs_redraw_complete) {
                ...
                List<Systems.Status> instances_applied = Shortcuts.LOCAL_PLAYER.Thing.Statuses.GetAllApply();
                ...
                foreach (Systems.Status status in instances_applied) {
                    ...
                    if (status.ID != id_skip) {
                        switch (status.Specific_UI_Type) {
                            ...
                            default:
                                Utilities.Logger.Error("Unsupported UI_TYPES: " + status.Specific_UI_Type);
                                break;
                        }
                    }
                }
