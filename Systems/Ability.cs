...
namespace ExperienceAndClasses.Systems {
    public abstract class Ability {
        ...
        /// default is none
        /// </summary>
        public Systems.PlayerClass.IDs Specific_Required_Class_ID { get; protected set; } = Systems.PlayerClass.IDs.None;
        ...
        public void LoadTexture() {
            if (specific_texture_path != null) {
                Texture = ModContent.GetTexture(specific_texture_path);
            }
            else {
        ...
        public void AddHotkeyData(bool primary, bool alternate, int index) {
            ...
            if (alternate) {
                if (Shortcuts.HOTKEY_ALTERNATE_EFFECT.GetAssignedKeys().Count > 0) {
                    hotkey += Shortcuts.HOTKEY_ALTERNATE_EFFECT.GetAssignedKeys()[0] + " + ";
                }
                else {
                    hotkey_not_set = true;
                    hotkey += "Alternate + ";
            ...
            ModHotKey key;
            if (primary) {
                key = Shortcuts.HOTKEY_ABILITY_PRIMARY[index];
            }
            else {
                key = Shortcuts.HOTKEY_ABILITY_SECONDARY[index];
            }
            hotkeys.Add(key);
        ...
        public void Activate(bool show_message = true) {
            ...
            if (cooldown_seconds > 0) {
                Time_Cooldown_End = Shortcuts.Now.AddSeconds(cooldown_seconds);
            ...
            if (base_cost_mana > 0) {
                ...
                Shortcuts.LOCAL_PLAYER.UseMana(cost_mana);
            }
        ...
        private void UpdateTooltip() {
            Tooltip = "Required Class: " + Systems.PlayerClass.LOOKUP[(byte)Specific_Required_Class_ID].Name + "\n" +
        ...
        public float CooldownPercent() {
            float seconds_remaining = (float)Math.Max(0f, Time_Cooldown_End.Subtract(Shortcuts.Now).TotalSeconds);
            return seconds_remaining / cooldown_seconds;
        }

        public Color Colour {
            get {
                return Systems.PlayerClass.LOOKUP[(byte)Specific_Required_Class_ID].Colour;
            }
        }
        ...
        private ushort CalculateManaCost() {
            //mana cost
            base_cost_mana = (ushort)ModifyCostManaBase(specific_mana_cost_flat + (specific_mana_cost_percent * Shortcuts.LOCAL_PLAYER.player.statManaMax2));
            if (specific_mana_apply_reduction) {
                cost_mana = (ushort)Math.Max(0, ModifyCostManaFinal(base_cost_mana * Shortcuts.LOCAL_PLAYER.player.manaCost));
            }
            else {
        ...
        private float CalculateCooldown() {
            ...
            if (specific_cooldown_apply_reduction) {
                ...
                cooldown_seconds /= Shortcuts.LOCAL_PLAYER.CSheet.Stats.Ability_Delay_Reduction;
            }
        ...
        private void CalculatePosition() {
            ...
            switch (specific_target_position_type) {
                ...
                case TARGET_POSITION_TYPE.BETWEEN_SELF_AND_CURSOR:
                    ...
                    break;
                default:
                    Utilities.Logger.Error("Unsupported TARGET_POSITION_TYPE in [" + ID + "]: " + specific_target_position_type);
                    return;
            }
        }
        ...
        private float CalculatePower() {
            ...
            for (byte i = 0; i<(byte)Systems.Attribute.IDs.NUMBER_OF_IDs; i++) {
                if (Systems.Attribute.LOOKUP[i].Active) { //must be an active attribute
                    multiplier += specific_power_attribute_multipliers[i] * Shortcuts.LOCAL_PLAYER.CSheet.Attributes.Final[i];
                }
            }
        ...
        private byte CalculateLevel() {
            ...
            byte levels = 0;
            Systems.PlayerClass c = Shortcuts.LOCAL_PLAYER.CSheet.Class_Primary.Class;
            if (c.ID == Specific_Required_Class_ID) {
                level_primary = Shortcuts.LOCAL_PLAYER.CSheet.Class_Primary.Level_Active;
            }
            else {
                levels = Shortcuts.LOCAL_PLAYER.CSheet.Class_Primary.Level_Active;
                c = c.Prereq;
                while (c != null) {
                    levels += c.Max_Level;
                    if (c.ID == Specific_Required_Class_ID) {
            ...
            levels = 0;
            c = Shortcuts.LOCAL_PLAYER.CSheet.Class_Secondary.Class;
            if (c.ID == Specific_Required_Class_ID) {
                level_secondary = Shortcuts.LOCAL_PLAYER.CSheet.Class_Secondary.Level_Active;
            }
            else {
                levels = Shortcuts.LOCAL_PLAYER.CSheet.Class_Secondary.Level_Active;
                c = c.Prereq;
                while (c != null) {
                    levels += c.Max_Level;
                    if (c.ID == Specific_Required_Class_ID) {
        ...
        private void CalculateTargets() {
            ...
            Utilities.Containers.Thing self = Shortcuts.LOCAL_PLAYER.Thing;
        ...
        private USE_RESULT PreActivate() {
            ...
            if (!Shortcuts.LOCAL_PLAYER.CSheet.Stats.Can_Use_Abilities) {
            ...
            //channelling
            if (!specific_can_be_used_while_channelling && Shortcuts.LOCAL_PLAYER.CSheet.Stats.Channelling) {
            ...
            if (CalculateResourceCost() > 0) {
                ...
                if (!EACPlayer.Resources.ContainsKey(specific_resource)) {
        ...
        private void FailMessage(USE_RESULT result) {
            if (Shortcuts.LOCAL_PLAYER.show_ability_fail_messages.value) {

                if ((result != type_last_Fail_message) || (Shortcuts.Now.CompareTo(time_allow_repeat_fail_message) > 0)) {

                    type_last_Fail_message = result;
                    time_allow_repeat_fail_message = Shortcuts.Now.AddSeconds(time_between_repeat_fail_message_seconds);
                    ...
                    switch (result) {
                        ...
                        case USE_RESULT.SUCCESS:
                            Utilities.Logger.Error("FailMessage called for USE_RESULT.SUCCESS for [" + ID + "]: " + result);
                            return;

                        case USE_RESULT.FAIL_MISSING_RESOURCE:
                            Utilities.Logger.Error("Missing resource for [" + ID + "]: " + specific_resource);
                            return;

                        default:
                            Utilities.Logger.Error("Unsupported USE_RESULT for [" + ID + "]: " + result);
                            return;
                    }
        ...
        protected static bool CheckPassiveSetVar<T>(Systems.Passive.IDs id, out T variable, T value_if_has_passive, T value_default) {
            if (Shortcuts.LOCAL_PLAYER.Passives.ContainsUnlocked(id)) {
        ...
        /// <summary>
        /// called after repopulating EACPlayer passives
        /// </summary>
        ...
        public class Warrior_Block : Ability {
            ...
            public Warrior_Block() : base(IDs.Warrior_Block) {
                ...
                Specific_Required_Class_ID = Systems.PlayerClass.IDs.Warrior;
            ...
            protected override void DoEffectTargetFriendly(Thing target) {
                float defense_bonus = Math.Min(level, Shortcuts.LOCAL_PLAYER.Attributes_Final[(byte)Systems.Attribute.IDs.Vitality] / 2f);
