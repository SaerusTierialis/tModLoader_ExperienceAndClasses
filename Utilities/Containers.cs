...
namespace ExperienceAndClasses.Utilities.Containers {
    ...
    public class TimeSortedStatusList : List<Systems.Status> {
        ...
        public new void Add(Systems.Status status) {
            //is full?
            if (Count == Capacity) {
                Logger.Error("A status icon cannot be displayed because there are not enough slots!");
                return;
            }
    ...
    public class LevelSortedPassives : List<Systems.Passive> {
        ...
        public new void Add(Systems.Passive passive) {
            ...
            for (int i = 0; i < Count; i++) {
                tier_prior = Systems.PlayerClass.LOOKUP[(byte)this[i].Specific_Required_Class_ID].Tier;
                tier_new = Systems.PlayerClass.LOOKUP[(byte)passive.Specific_Required_Class_ID].Tier;
    ...
    public class Thing {
        ...
        public Thing(EACPlayer eacplayer) {
            Is_Player = true;
            Is_Npc = false;
        ...
        public EACPlayer EACPlayer {
            get {
                if (EACPlayer == null) {
                    Logger.Error("Attempted to get EACPlayer from non-player Thing!");
                    return null;
                }
                else {
                    return EACPlayer;
        ...
        public EACNPC EACNPC {
            get {
                if (eacnpc == null) {
                    Logger.Error("Attempted to get EACNPC from non-NPC Thing!");
                    return null;
                }
                else {
                    return eacnpc;
        ...
        public bool HasBuff(int buff_id) {
            if (Is_Player) {
                ...
                Utilities.Logger.Error("Unknown thing type");
                return false;
            }
        }
    ...
    public class StatusList {
        ...
        public bool Add(Systems.Status status) {
            ...
            if (statuses.TryGetValue(status.ID, out status_instances)) {
                ...
                Logger.Error("Failed to create StatusInstances for new status " + status.Specific_Name);
                return false;
            }
        }
        ...
        public void Remove(Systems.Status.IDs status_id, byte instance_id) {
            ...
            if (statuses.TryGetValue(status_id, out status_instances)) {
                ...
                Logger.Error("Failed to find StatusInstances for removing status " + status_id + " (instance " + instance_id + ")");
        }
        ...
        public void DeleteAll(Systems.Status.IDs status_id) {
            if (!statuses.Remove(status_id)) {
                Utilities.Logger.Error("Tried to delete all instances of [" + status_id + "] when there were none!");
            }
        }
        ...
        public List<Systems.Status> GetAllApply() {
            ...
            foreach (StatusInstances status_type in statuses.Values) {
                ...
                switch (instances[0].Specific_Apply_Type) {
                    ...
                    default:
                        Utilities.Logger.Error("Unsupported APPLY_TYPES:" + instances[0].Specific_Apply_Type);
                        break;
                }
        ...
        private class StatusInstances {
            ...
            private bool AddNewStatus(Systems.Status status) {
                if (status.Instance_ID != UNASSIGNED_INSTANCE_KEY) {
                    ...
                    Utilities.Logger.Error("AddNewStatus called incorrectly!");
                    return false;
                }
                else {
                    ...
                    switch (limit_type) {
                        case Systems.Status.LIMIT_TYPES.MANY:
                            ...
                            if (key_temp < 0) {
                                ...
                                Utilities.Logger.Error("Status instance limit reached for [" + status.ID + "] (this probably means that something went wrong, please report)");
                                return false;
                            }
                            break;
                        ...
                        default:
                            ...
                            Utilities.Logger.Error("Unsupported limit type: " + limit_type);
                            return false;
                    }
                    ...
                    if (instances.ContainsKey(key)) {
                        ...
                        if (instances.TryGetValue(key, out existing)) {
                            ...
                            Utilities.Logger.Error("Failed to get existing status: " + status.Specific_Name);
                            return false;
                        }
                    }
