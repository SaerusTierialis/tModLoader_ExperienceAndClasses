    ...
    public static class Legacy {
        ...
        public class MyPlayer : ModPlayer {
            ...
            public override void Load(TagCompound tag) {
                legacy_xp = Utilities.Commons.TagTryGet<double>(tag, "experience", 0);
                legacy_xp_spent = Utilities.Commons.TagTryGet<double>(tag, "experience_spent", 0);
            }
                ...
                for (int loop = 0; loop <= 1; loop++) { //loop twice in case first loop clears up enough space for other conversions
                    foreach (Item item in player.inventory) {
                        if (item.type > 0 && item.Name.Equals("Unloaded Item")) {
                            ...
                            string mod = Utilities.Commons.TagTryGet<String>(tag, "mod", "");

                            if (mod.Equals("ExperienceAndClasses")) {

                                string name = Utilities.Commons.TagTryGet<String>(tag, "name", "");
                                ...
                                switch (name) {
                                    case "Monster_Orb":
                                        items = new ModItem[] { ModContent.GetInstance<Items.Orb_Monster>() };
                                        break;

                                    case "Boss_Orb":
                                        items = new ModItem[] { ModContent.GetInstance<Items.Orb_Boss>() };
                                        break;
                                    ...
                                    case "ClassToken_Cleric":
                                        items = new ModItem[] { ModContent.GetInstance<Items.Unlock_Tier2>() };
                                        break;
                                    ...
                                    case "ClassToken_Archer":
                                        items = new ModItem[] { ModContent.GetInstance<Items.Unlock_Tier2>(), ModContent.GetInstance<Items.Unlock_Tier3>() };
                                        break;
