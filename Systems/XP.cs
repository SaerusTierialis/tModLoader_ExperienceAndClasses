    ...
    static class XP {
        ...
        public static class Requirements {
            ...
            public static void SetupXPRequirements() {
                ...
                int levels = PlayerClass.TIER_MAX_LEVELS[1] + PlayerClass.TIER_MAX_LEVELS[2] + PlayerClass.TIER_MAX_LEVELS[3];
            ...
            /// <returns></returns>
            public static uint GetXPReq(PlayerClass c, byte level) {
        ...
        public static class Adjusting {
            ...
            public static bool LocalCanGainXPPrimary {
                get {
                    return (Shortcuts.LOCAL_PLAYER.Class_Primary.Tier > 0) && (Shortcuts.LOCAL_PLAYER.Class_Levels[Shortcuts.LOCAL_PLAYER.Class_Primary.ID_num] < Shortcuts.LOCAL_PLAYER.Class_Primary.Max_Level);
                }
            }
            ...
            public static bool LocalCanGainXPSecondary {
                get {
                    return Shortcuts.LOCAL_PLAYER.Allow_Secondary && (Shortcuts.LOCAL_PLAYER.Class_Secondary.Tier > 0) && (Shortcuts.LOCAL_PLAYER.Class_Levels[Shortcuts.LOCAL_PLAYER.Class_Secondary.ID_num] < Shortcuts.LOCAL_PLAYER.Class_Secondary.Max_Level);
                }
            }
            ...
            public static void LocalAddXP(uint xp, bool can_show_xp_message = true) {
                if (can_show_xp_message && Shortcuts.LOCAL_PLAYER.show_xp.value) {
                ...
                if (LocalCanGainXP) {
                    ...
                    if (add_primary && add_secondary) {
                        LocalAddXPToClass(Shortcuts.LOCAL_PLAYER.Class_Primary.ID_num, (uint)Math.Ceiling(xp * Systems.XP.SUBCLASS_PENALTY_XP_MULTIPLIER_PRIMARY));
                        LocalAddXPToClass(Shortcuts.LOCAL_PLAYER.Class_Secondary.ID_num, (uint)Math.Ceiling(xp * Systems.XP.SUBCLASS_PENALTY_XP_MULTIPLIER_SECONDARY));
                    }
                    else if (add_primary) {
                        LocalAddXPToClass(Shortcuts.LOCAL_PLAYER.Class_Primary.ID_num, xp);
                    }
                    else if (add_secondary) {
                        LocalAddXPToClass(Shortcuts.LOCAL_PLAYER.Class_Secondary.ID_num, xp);
                    }
                    else {
                        //shouldn't be reachable unless something is changed later
                        Shortcuts.LOCAL_PLAYER.extra_xp = Math.Max(Shortcuts.LOCAL_PLAYER.extra_xp, Shortcuts.LOCAL_PLAYER.extra_xp + xp); //prevent overflow
                        return;
                    }

                    if (LocalCheckDoLevelup()) {
                        //levelup!
                        Systems.Status.LevelUp.CreateNew(Shortcuts.LOCAL_PLAYER.Thing);
                    }
                }
                else {
                    Shortcuts.LOCAL_PLAYER.extra_xp = Math.Max(Shortcuts.LOCAL_PLAYER.extra_xp, Shortcuts.LOCAL_PLAYER.extra_xp + xp); //prevent overflow
            ...
            public static void LocalDeathXP() {
                EACPlayer local_player = Shortcuts.LOCAL_PLAYER;

                if (!local_player.AFK && LocalCanGainXP) {
                    uint xp_loss;

                    //primary
                    Systems.PlayerClass c = local_player.Class_Primary;
            ...
            public static void LocalAddXPToClass(byte class_id, uint amount) {
                Shortcuts.LOCAL_PLAYER.Class_XP[class_id] = Math.Max(Shortcuts.LOCAL_PLAYER.Class_XP[class_id] + amount, Shortcuts.LOCAL_PLAYER.Class_XP[class_id]);
            }
            ...
            public static void LocalSubtractXPFromClass(byte class_id, uint amount) {
                Shortcuts.LOCAL_PLAYER.Class_XP[class_id] = Math.Min(Shortcuts.LOCAL_PLAYER.Class_XP[class_id] - amount, Shortcuts.LOCAL_PLAYER.Class_XP[class_id]);
            }
            ...
            private static bool LocalCheckDoLevelup() {
                ...
                byte effective_primary = Shortcuts.LOCAL_PLAYER.Class_Primary_Level_Effective;
                byte effective_secondary = Shortcuts.LOCAL_PLAYER.Class_Secondary_Level_Effective;

                //level up
                Shortcuts.LOCAL_PLAYER.Class_Primary.LocalCheckDoLevelup();
                Shortcuts.LOCAL_PLAYER.Class_Secondary.LocalCheckDoLevelup();

                //adjust effective levels
                EACPlayer.LocalCalculateEffectiveLevels();

                //update class info if needed
                if ((effective_primary != Shortcuts.LOCAL_PLAYER.Class_Primary_Level_Effective) || (effective_secondary != Shortcuts.LOCAL_PLAYER.Class_Secondary_Level_Effective)) {
