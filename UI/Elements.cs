...
namespace ExperienceAndClasses.UI {
    ...
    class XPBar : UIElement {
        ...
        public void Update() {
            if (visible) {
                uint xp = Shortcuts.LOCAL_PLAYER.CSheet.Classes.XP[Class_Tracked.ID_num];
                uint xp_needed = Systems.XP.Requirements.GetXPReq(Class_Tracked, Shortcuts.LOCAL_PLAYER.CSheet.Classes.Levels[Class_Tracked.ID_num]);
    ...
    class AttributeText : UIPanel {
        ...
        public void ClickAdd(UIMouseEvent evt, UIElement listeningElement) {
            ...
            for (int i = 0; i < point_add; i++) {
                if (!Shortcuts.LOCAL_PLAYER.CSheet.Attributes.AllocatePoint(attribute.ID_num)) {
                    break;
                }
            }
        }
        ...
        public void Update() {
            string str = "" + Shortcuts.LOCAL_PLAYER.CSheet.Attributes.Final[attribute.ID_num];
            final.SetText(str);
            final.Left.Set(left_final - (Main.fontMouseText.MeasureString(str).X * scale), 0f);

            int allocation_cost = Systems.Attribute.AllocationPointCost(Shortcuts.LOCAL_PLAYER.CSheet.Attributes.Allocated[attribute.ID_num]);
            cost.SetText("" + allocation_cost);

            float width_cutoff = final.Left.Pixels - sum.Left.Pixels - 5f;

            str = Shortcuts.LOCAL_PLAYER.CSheet.Attributes.Allocated_Effective[attribute.ID_num] + "+" +
                    (Shortcuts.LOCAL_PLAYER.CSheet.Attributes.From_Class[attribute.ID_num] + Shortcuts.LOCAL_PLAYER.CSheet.Attributes.From_Other[attribute.ID_num]);
            ...
            if (allocation_cost <= Shortcuts.LOCAL_PLAYER.CSheet.Attributes.Points_Available) {
    ...
    class ClassButton : UIImageButton {
        ...
        public override void Click(UIMouseEvent evt) {
            ...
            if (!Shortcuts.LOCAL_PLAYER.CSheet.Classes.Unlocked[Class.ID_num]) {
                UIPopup.Instance.ShowUnlockClass(this, Class);
            }
            else {
                if (Shortcuts.LOCAL_PLAYER.CSheet.Class_Primary.Class.ID_num == Class.ID_num) {
                    Systems.PlayerClass.LOOKUP[(byte)Systems.PlayerClass.IDs.None].LocalTrySetClass(true);
                }
                else {
        ...
        public override void RightClick(UIMouseEvent evt) {
            ...
            if(!Shortcuts.LOCAL_PLAYER.CSheet.Class_Secondary_Unlocked) {
                UIPopup.Instance.ShowUnlockSubclass(this);
            }
            else if (Shortcuts.LOCAL_PLAYER.CSheet.Class_Secondary.Class.ID_num == Class.ID_num) {
                Systems.PlayerClass.LOOKUP[(byte)Systems.PlayerClass.IDs.None].LocalTrySetClass(false);
            }
            else {
        ...
        public void Update() {
            byte level = Shortcuts.LOCAL_PLAYER.CSheet.Classes.Levels[Class.ID_num];
            if (Shortcuts.LOCAL_PLAYER.CSheet.Classes.Unlocked[Class.ID_num]) {
                //not locked
                ...
                if ((Shortcuts.LOCAL_PLAYER.CSheet.Class_Primary.Class.ID_num == Class.ID_num) || (Shortcuts.LOCAL_PLAYER.CSheet.Class_Secondary.Class.ID_num == Class.ID_num)) {
                    //selected
