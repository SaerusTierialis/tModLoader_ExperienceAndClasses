...
namespace ExperienceAndClasses.UI {
    class UIHelpSettings : UIStateCombo {
        ...
        protected override void InitializeState() {
            ...
            AddHelpTextPanel("Nearby Targets", "Some classes gain a damage bonus again nearby targets. This refers to any hit that occurs within a distance of " + EACPlayer.DISTANCE_CLOSE_RANGE + ". For reference, this distance is equal to the width of " + (EACPlayer.DISTANCE_CLOSE_RANGE / Main.LocalPlayer.width) + " players.\n\n" + 
            ...
            AddHelpTextPanel("Hotkeys", "This mod makes use of several hotkeys that need to be set before they can be used. To set these: click Settings either in the main menu or in the inventory screen, select Controls, scroll down to the Mod Controls section, and set each key (or click default).\n\n" +
                "There are " + Shortcuts.NUMBER_ABILITY_SLOTS_PER_CLASS + " ability keys for the primary and secondary class. Some classes have more than " + Shortcuts.NUMBER_ABILITY_SLOTS_PER_CLASS +
            ...
            AddHelpTextPanel("AFK", "Away From Keyboard (AFK) triggers if you do not use any controls for " + EACPlayer.AFK_SECONDS + " seconds. When AFK, you cannot gain or lose XP.", ref sorted_items);
            AddHelpTextPanel("In Combat", "In Combat triggers for " + EACPlayer.IN_COMBAT_SECONDS + " seconds each time you take or deal damage.", ref sorted_items);
            ...
            sorted_items.Clear();
            AddSettingsToggle(Shortcuts.LOCAL_PLAYER.show_xp, ref sorted_items);
            AddSettingsToggle(Shortcuts.LOCAL_PLAYER.show_ability_fail_messages, ref sorted_items);
            AddSettingsToggle(Shortcuts.LOCAL_PLAYER.show_classes_button, ref sorted_items);
