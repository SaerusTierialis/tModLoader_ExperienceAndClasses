...
namespace ExperienceAndClasses.Systems {
    public abstract class PlayerClass {
        ...
        public static PlayerClass[] LOOKUP { get; private set; }
        ...
        public Systems.Ability.IDs[] AbilitiesID { get; protected set; } = Enumerable.Repeat(Systems.Ability.IDs.NONE, Shortcuts.NUMBER_ABILITY_SLOTS_PER_CLASS).ToArray();
        public Systems.Ability.IDs[] AbilitiesID_Alt { get; protected set; } = Enumerable.Repeat(Systems.Ability.IDs.NONE, Shortcuts.NUMBER_ABILITY_SLOTS_PER_CLASS).ToArray();
        ...
        private CLASS_VALIDITY LocalCheckClassValid(bool is_primary) {
            if (Shortcuts.LOCAL_PLAYER.IN_COMBAT) {
                ...
                if (is_primary) {
                    class_same_slot = Shortcuts.LOCAL_PLAYER.Class_Primary;
                    class_other_slot = Shortcuts.LOCAL_PLAYER.Class_Secondary;
                }
                else {
                    class_same_slot = Shortcuts.LOCAL_PLAYER.Class_Secondary;
                    class_other_slot = Shortcuts.LOCAL_PLAYER.Class_Primary;
                }

                if (((Shortcuts.LOCAL_PLAYER.Class_Levels[ID_num] <= 0) || !Shortcuts.LOCAL_PLAYER.Class_Unlocked[ID_num]) && (ID_num != (byte)Systems.PlayerClass.IDs.None)) {
        ...
        public bool LocalTrySetClass(bool is_primary) {
            ...
            if (!is_primary && !Shortcuts.LOCAL_PLAYER.Allow_Secondary) {
            ...
            byte id_other;
            if (is_primary) {
                id_other = Shortcuts.LOCAL_PLAYER.Class_Secondary.ID_num;
            }
            else {
                id_other = Shortcuts.LOCAL_PLAYER.Class_Primary.ID_num;
            }
            if ((ID_num == id_other) && (ID_num != (byte)Systems.PlayerClass.IDs.None)) {
                ...
                switch (valid) {
                    case Systems.PlayerClass.CLASS_VALIDITY.VALID:
                        ...
                        //destroy all minions
                        Shortcuts.LOCAL_PLAYER.CheckMinions();
                        if (Shortcuts.LOCAL_PLAYER.minions.Count > 0) {
                            Main.NewText("Your minions have been despawned because you changed classes!", UI.Constants.COLOUR_MESSAGE_ERROR);
                            foreach (Projectile p in Shortcuts.LOCAL_PLAYER.minions) {
                                p.Kill();
                            }
                        }

                        EACPlayer.LocalForceClass(this, is_primary);
                        return true;
        ...
        private static void LocalSwapClass() {
            EACPlayer.LocalForceClasses(Shortcuts.LOCAL_PLAYER.Class_Secondary, Shortcuts.LOCAL_PLAYER.Class_Primary);
            EACPlayer.LocalUpdateAll();
        }
        ...
        public bool LocalTryUnlockClass() {
            //check locked
            if (Shortcuts.LOCAL_PLAYER.Class_Unlocked[ID_num]) {
            ...
            if (Tier == 3 && !LocalCanUnlockTier3()) {
                if (!Shortcuts.LOCAL_PLAYER.Defeated_WOF) {
            ...
            //unlock class
            Shortcuts.LOCAL_PLAYER.Class_Unlocked[ID_num] = true;
            if (Shortcuts.LOCAL_PLAYER.Class_Levels[ID_num] < 1) {
                Shortcuts.LOCAL_PLAYER.Class_Levels[ID_num] = 1;
            }
            ...
            uint extra_xp_add = (uint)(Shortcuts.LOCAL_PLAYER.extra_xp * Systems.XP.EXTRA_XP_POOL_MULTIPLIER);
            if (extra_xp_add > 0) {
                //add xp
                Systems.XP.Adjusting.LocalAddXPToClass(ID_num, extra_xp_add);

                //clear pool
                Shortcuts.LOCAL_PLAYER.extra_xp = 0;
        ...
        public bool LocalCheckDoLevelup(bool announce = true) {
            ...
            while (Shortcuts.LOCAL_PLAYER.Class_Levels[ID_num] < Max_Level) {
                xp_req = Systems.XP.Requirements.GetXPReq(this, Shortcuts.LOCAL_PLAYER.Class_Levels[ID_num]);
                if (Shortcuts.LOCAL_PLAYER.Class_XP[ID_num] < xp_req) {
                    break;
                }
                else {
                    Systems.XP.Adjusting.LocalSubtractXPFromClass(ID_num, xp_req);
                    Shortcuts.LOCAL_PLAYER.Class_Levels[ID_num]++;
                    if (announce) {
                        LocalAnnounceLevel();
                    }
                    any_levels = true;
        ...
        private void LocalAnnounceLevel() {
            ...
            if (!Shortcuts.IS_SERVER) {
                byte level = Shortcuts.LOCAL_PLAYER.Class_Levels[ID_num];
        ...
        public bool LocalHasClassPrereq() {
            ...
            while (pre != null) {
                if (Shortcuts.LOCAL_PLAYER.Class_Levels[pre.ID_num] < pre.Max_Level) {
        ...
        public static void LocalTryUnlockSubclass() {
            //check locked
            if (Shortcuts.LOCAL_PLAYER.Allow_Secondary) {
                Utilities.Logger.Error("Trying to unlock multiclassing when already unlocked");
            }
            else {
                //item requirements
                Item item = GetInstance<Items.Unlock_Subclass>().item;
                if (!Shortcuts.LOCAL_PLAYER.player.HasItem(item.type)) {
                    ...
                    //unlock class
                    Shortcuts.LOCAL_PLAYER.Allow_Secondary = true;
        ...
        public Systems.Ability[] GetAbilities(bool primary, bool alternate) {
            ...
            Systems.Ability[] abilities = new Systems.Ability[Shortcuts.NUMBER_ABILITY_SLOTS_PER_CLASS];
        ...
        public static bool LocalCanUnlockTier3() {
            return Shortcuts.LOCAL_PLAYER.Defeated_WOF;
        }
        ...
        public abstract class Tier2 : RealClass {
            public Tier2(IDs id, PowerScaling.IDs power_scaling) : base(id, power_scaling) {
                Tier = 2;
                Unlock_Item = GetInstance<Items.Unlock_Tier2>();
                Max_Level = TIER_MAX_LEVELS[Tier];
        ...
        public abstract class Tier3 : RealClass {
            public Tier3(IDs id, PowerScaling.IDs power_scaling, IDs prereq) : base(id, power_scaling) {
                Tier = 3;
                Unlock_Item = GetInstance<Items.Unlock_Tier3>();
                Max_Level = TIER_MAX_LEVELS[Tier];
        ...
        public class Explorer : Tier2 {
            public Explorer() : base(IDs.Explorer, PowerScaling.IDs.NonCombat) {
                ...
                Unlock_Item = GetInstance<Items.Unlock_Explorer>();
