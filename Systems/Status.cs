    ...
    public abstract class Status {
        ...
        public void LoadTexture() {
            if (specific_texture_path != null) {
                ...
                Textures.Add(ModContent.GetTexture(specific_texture_path));
            }
        }
        ...
        public void RemoveEverywhere() {
            ...
            if (Specific_Syncs && !Shortcuts.IS_SINGLEPLAYER) {
                if (Owner.Local) {
                    //sending local status (could be server if a status is evere created there)
                    Utilities.PacketHandler.RemoveStatus.Send(this, Shortcuts.WHO_AM_I);
                }
                else if (Shortcuts.IS_SERVER) {
        ...
        public bool Update() {
            ...
            if (remove != CHECK_REMOVE.DONT_REMOVE) {
                ...
                if (Specific_Target_Channelling && Target.Is_Player) {
                    Target.EACPlayer.channelling = true;
                }
        ...
        public String GetIconDurationString() {
            if (Specific_Duration_Type == DURATION_TYPES.TIMED) {
                ...
                TimeSpan time_remain = Time_End.Subtract(Shortcuts.Now);
        ...
        private CHECK_REMOVE CheckRemoval() {
            ...
            if (local_enforce_duration && (Shortcuts.Now.CompareTo(Time_End) >= 0)) { //timeup
            ...
            if (Owner.Is_Player && Owner.Local) { //owner is always player if locally_owned
                ...
                //required passive
                if ((specific_owner_player_required_passive != Systems.Passive.IDs.NONE) && !Owner.EACPlayer.Passives.ContainsUnlocked(specific_owner_player_required_passive)) {
                    return CHECK_REMOVE.REMOVE_OWNER_REQUIRED_PASSIVE;
                }

                //required ability
                if ((specific_owner_player_required_ability != Systems.Ability.IDs.NONE) && !Owner.EACPlayer.HasAbility(specific_owner_player_required_ability)) {
        ...
        public bool Merge(Status existing) {
            ...
            if (!existing.Owner.Local) {
                Utilities.Logger.Error("Merge status called by non-owner!");
                return false;
            }
        ...
        public void DoEffect() {
            ...
            if (Specific_Duration_Type == DURATION_TYPES.INSTANT) {
                ...
                if ((Specific_Effect_Type == EFFECT_TYPES.TIMED) || (Specific_Effect_Type == EFFECT_TYPES.CONSTANT_AND_TIMED)) {
                    if (Shortcuts.Now.CompareTo(Times_Next_Timed_Effect[ID]) >= 0) {
                        //call effect
                        EffectTimed();
        ...
        /// <summary>
        /// Adds AutoPassive to local EACPlayer
        /// </summary>
        public void AddAutoPassive(Utilities.Containers.Thing self) {
            if (Shortcuts.IS_SERVER) {
                Utilities.Logger.Error("Called AddAutoPassive on server for [" + ID + "]");
            }
            else if (!Specific_Is_AutoPassive) {
                Utilities.Logger.Error("Called AddAutoPassive for non-AutoPassive [" + ID + "]");
            }
            else if (!self.Local) {
                Utilities.Logger.Error("Called AddAutoPassive for non-local self for [" + ID + "]");
            }
            else {
                Add(ID, self, self);
            }
        ...
        public float GetData(AUTOSYNC_DATA_TYPES key, float default_value = 0f) {
            ...
            if (!autosync_data.TryGetValue(key, out value)) {
                Utilities.Logger.Error("Status attempted to access invalid sync data: " + ID + " " + key);
            }
            return value;
        }
        ...
        public static void Add(IDs id, Utilities.Containers.Thing target, Utilities.Containers.Thing owner, Dictionary<AUTOSYNC_DATA_TYPES, float> sync_data = null, float seconds_remaining = 0f, float seconds_until_effect = 0f, byte instance_id = Utilities.Containers.StatusList.UNASSIGNED_INSTANCE_KEY, BinaryReader reader = null, bool set_statuses = false) {
            ...
            if ((target.Is_Player && !status.specific_target_can_be_player) || (target.Is_Npc && !status.specific_target_can_be_npc)) {
                Utilities.Logger.Error("Invalid target for " + status.Specific_Name);
                return;
            }
            ...
            if ((owner.Is_Player && !status.specific_owner_can_be_player) || (owner.Is_Npc && !status.specific_owner_can_be_npc)) {
                Utilities.Logger.Error("Invalid owner for " + status.Specific_Name);
                return;
            }
            ...
            switch (status.Specific_Duration_Type) {
                case (DURATION_TYPES.TIMED):
                    ...
                    status.Time_End = Shortcuts.Now.AddSeconds(seconds_remaining);
                    break;
                ...
                default:
                    Utilities.Logger.Error("Unsupported DURATION_TYPES: " + status.Specific_Duration_Type);
                    break;
            }
            ...
            if ((status.Specific_Effect_Type == EFFECT_TYPES.TIMED) || (status.Specific_Effect_Type == EFFECT_TYPES.CONSTANT_AND_TIMED)) {
                ...
                if (seconds_until_effect != 0) {
                    //use provided time
                    time_next = Shortcuts.Now.AddSeconds(seconds_until_effect);
                }
                else {
                    //no provided time so do first effect asap
                    time_next = Shortcuts.Now;
                }
            ...
            if (!set_statuses && status.Specific_Syncs && !Shortcuts.IS_SINGLEPLAYER) {
                if (owner.Local) {
                    //sending local status (could be server if a status is ever created there - npc owned status)
                    Utilities.PacketHandler.AddStatus.Send(status, Shortcuts.WHO_AM_I);
                }
                else if (Shortcuts.IS_SERVER) {
                    //not local, relaying status to clients through server (can assume owner is player and is origin)
        ...
        public abstract class ChannelManaCostSync : Status {
            ...
            protected override void OnStart() {
                if (apply_mana_cost_reduction) {
                    specific_timed_effect_sec /= Target.EACPlayer.player.manaCost;
                }
            }

            protected override void EffectTimed() {
                if (Target.Local) {
                    if (!Target.EACPlayer.UseMana(mana_cost_per_timed_effect)) {
                        RemoveEverywhere();
                    }
                }
            }
        ...
        public class LevelUp : InstantSync {
            ...
            protected override void OnStart() {
                int life = Target.EACPlayer.player.statLifeMax2 - Target.EACPlayer.player.statLife;
                int mana = Target.EACPlayer.player.statManaMax2 - Target.EACPlayer.player.statMana;

                if (life > 0)
                    Target.EACPlayer.player.HealEffect(life, false);
                if (mana > 0)
                    Target.EACPlayer.player.ManaEffect(mana);

                Target.EACPlayer.player.statLife = Target.EACPlayer.player.statLifeMax2;
                Target.EACPlayer.player.statMana = Target.EACPlayer.player.statManaMax2;
        ...
        public class Warrior_Block : ChannelManaCostSync {
            ...
            protected override void EffectConstant() {
                Target.EACPlayer.player.statDefense += (int)Magitude1;
            }
