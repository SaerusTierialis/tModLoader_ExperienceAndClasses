...
namespace ExperienceAndClasses.Systems {
    public abstract class Passive {
        ...
        /// default is none
        /// </summary>
        public Systems.PlayerClass.IDs Specific_Required_Class_ID { get; protected set; } = Systems.PlayerClass.IDs.New;
        ...
        public bool CorrectClass() {
            ...
            //unobtainable
            if (Specific_Required_Class_ID == PlayerClass.IDs.New) {
                return false;
            }

            //check primary
            Systems.PlayerClass c = Shortcuts.LOCAL_PLAYER.Class_Primary;
            if (c.ID == Specific_Required_Class_ID) {
                correct_class = true;
                if (Shortcuts.LOCAL_PLAYER.Class_Primary_Level_Effective >= Specific_Required_Class_Level) {
                    Unlocked = true;
                }
            }
            else {
            ...
            //check secondary
            c = Shortcuts.LOCAL_PLAYER.Class_Secondary;
            if (c.ID == Specific_Required_Class_ID) {
                correct_class = true;
                if (Shortcuts.LOCAL_PLAYER.Class_Secondary_Level_Effective >= Specific_Required_Class_Level) {
                    Unlocked = true;
                }
            }
            else {
        ...
        private void EnableResource() {
            if (specific_resource != Systems.Resource.IDs.NONE) {
                EACPlayer.Resources.Add(specific_resource, Systems.Resource.LOOKUP[(byte)specific_resource]);
                if (!EACPlayer.Resources_Prior.ContainsKey(specific_resource)) {
